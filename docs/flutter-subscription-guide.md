...
| ユーザー管理 | 解約・更新・試用の管理が必要 | 購入後は管理不要 |
| ストア管理 | 自動更新ロジックをプラットフォームが管理 | シンプル |
| 向いているケース | SaaS的機能・定期コンテンツ配信 | ゲームアイテム・機能アンロック |

### 手数料体系

| プラットフォーム | 標準手数料 | 小規模事業者優遇 | 条件 |
|---|---|---|---|
| App Store（iOS） | 30% | 15% | 年間売上が100万ドル以下 |
| Google Play（Android） | 30% | 15% | 初年度のサブスク収益 |

---

## 2. サブスクリプションの種類

IAPで設定できるサブスクリプションの種類：

| 種類 | 説明 | iOS | Android |
|---|---|---|---|
| 自動更新サブスクリプション | 月額・年額で期間終了時に自動更新される最も一般的な形式 | ○ | ○ |
| 非自動更新サブスクリプション | 手動で更新が必要。現在は廃止傾向で非推奨 | ○ | ○ |
| フリートライアル付き | 無料期間（7日・14日・30日など）終了後に課金開始 | ○ | ○ |
| 段階プラン（Tier） | ベーシック・スタンダード・プレミアムなど複数グレード | ○ | ○ |
| イントロダクトリーオファー | 初回ユーザー向け割引価格（期間限定） | ○ | ○ |
| ファミリー共有 | 1契約で家族メンバーと共有 | ○（Family Sharing） | ○（Family Library） |

### 推奨構成

新規アプリへの導入であれば、まず**自動更新サブスクリプション**（フリートライアル付き）の1プランから始めるのが最もシンプルです。
