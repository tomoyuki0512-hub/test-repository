...
#### RevenueCat（purchases_flutter）を推奨するケース

以下の項目で「はい」が多い場合は RevenueCat が適しています：

- [ ] バックエンドを持たない、または最小限にしたい
- [ ] 複数のサブスクリプションプランを管理する予定
- [ ] 収益・チャーン分析のダッシュボードが欲しい
- [ ] Webhook でバックエンドへイベント通知したい
- [ ] 将来的に Web / 他プラットフォームへの展開を考えている
- [ ] 実装工数を削減したい（iOS/Android の差異を気にしたくない）

#### in_app_purchase（Flutter公式）を推奨するケース

以下の項目で「はい」が多い場合は in_app_purchase が適しています：

- [ ] バックエンドが既にあり、App Store Server API / Google Play Developer API での検証を自前で行える
- [ ] 外部サービスへの依存を排除したい
- [ ] プラン構成がシンプル（1〜3プラン程度）
- [ ] 月間売上が増えてもコストを固定したい（RevenueCat の1%課金を避けたい）
- [ ] プラットフォーム固有の細かい挙動を制御したい

---

### 7-2. バックエンド有無の判断基準

| 要件 | バックエンド必要？ | RevenueCatで代替可？ |
|---|---|---|
| アプリ内のみで機能開放（オフライン利用） | 不要 | — |
| サーバーリソース（API等）へのアクセス制御 | **必要** | ○（Webhook連携） |
| 複数デバイス間での権利同期 | **必要** | ○（RevenueCatが管理） |
| 自前アカウントとサブスク状態の紐付け | **必要** | ○（Webhook連携） |
| 不正利用防止のサーバー側検証 | **必要** | ○（RevenueCatが検証） |

---

### 7-3. iOS / Android 両対応に必要な設定一覧

| 設定項目 | iOS | Android |
|---|---|---|
| ストアコンソールでのプロダクト登録 | App Store Connect | Google Play Console |
| Capabilities の有効化 | Xcode で In-App Purchase を有効化 | 不要（パッケージが自動処理） |
| SDK バージョン要件 | StoreKit2（iOS 15+）/ Xcode 26 SDK（2026年4月〜必須） | Billing Library 7+（2025年8月〜必須） |
| テストアカウントの準備 | App Store Connect で Sandbox テスター作成 | Google Play Console でライセンステスター登録 |
| テスト配布方法 | 実機 + Sandbox アカウント / Xcode StoreKit Testing | 内部テストトラックへアップロード後に実機テスト |
| 解約導線のUI実装 | `itms-apps://apps.apple.com/account/subscriptions` へのリンク | Google Play サブスク管理ページへのリンク（＋アプリ内解約フロー推奨） |
| 購入復元の実装 | `InAppPurchase.instance.restorePurchases()` | 同左（Android は購入状態が自動復元されることが多い） |
| フリートライアルの表示 | 購入前に価格・期間・条件を明示 | 同左 |
| 審査申請時の準備 | プロダクト説明・スクリーンショット・審査メモ | プロダクト説明文・コンテンツレーティング |

---

### 7-4. 実装優先度マトリクス（新規開発向け）

| 優先度 | 項目 | 担当 |
|---|---|---|
| 🔴 必須 | プロダクト登録（両ストア） | ストア管理者 |
| 🔴 必須 | 購入フローの実装（パッケージ選定後） | エンジニア |
| 🔴 必須 | 解約導線UIの実装 | エンジニア / デザイナー |
| 🔴 必須 | 購入復元の実装 | エンジニア |
| 🔴 必須 | テスト（Sandbox / テストトラック） | QA / エンジニア |
| 🟡 推奨 | サーバーサイド検証（自前 or RevenueCat） | エンジニア |
| 🟡 推奨 | フリートライアル設定 | ストア管理者 |
| 🟡 推奨 | リアルタイム通知（APNS V2 / RTDN）の受信設定 | エンジニア |
| 🟢 任意 | プロモーションオファーの設定 | ストア管理者 |
| 🟢 任意 | ファミリー共有の有効化 | ストア管理者 |
| 🟢 任意 | 収益分析ダッシュボードの整備 | エンジニア / RevenueCat |

---

## 8. IAPで利用できる支払い方法（ユーザー視点）

IAP での課金は、ユーザーが各ストアのアカウントに登録した支払い方法を通じて行われます。開発者側は支払い方法を制御できません。以下は日本国内で利用できる主な支払い方法です。

### 8-1. App Store（iOS）の支払い方法
...
### 8-2. Google Play（Android）の支払い方法
...
| 考慮点 | 内容 |
|---|---|
| 支払い失敗時のハンドリング | キャリア決済の上限超過やカード期限切れ等で課金失敗が起きる。アプリは「督促中（Grace Period）」状態を適切にハンドリングし、ユーザーに解決を促すUIが必要 |
| ギフトカード残高不足 | 残高不足で購入できない場合のエラー処理が必要 |
| 地域差 | 対応する支払い方法は国・地域によって異なる。グローバル展開時は注意が必要 |
| Grace Period（猶予期間） | App Store・Google Play ともに課金失敗後に一定期間の猶予（Grace Period）があり、その期間中もサブスクリプションを有効に保つ実装が求められる |

### 8-4. 支払い方法の制御可否

**IAPを使う限り、特定の支払い方法を禁止・指定することは不可能です。**

地域・法的規制によって「IAP以外の外部決済」を使う選択肢は広がってきていますが、開発者・ユーザー双方に大きなトレードオフがあります。
...
## 9. Grace Period / Billing Retry / Account Hold 詳細
...
### 9-3. Grace Period が適用される決済タイミング
