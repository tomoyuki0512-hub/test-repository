...
| **米国** | 2025年4月の裁判所命令により外部決済リンクが許可 | Epic訴訟和解（2026年3月）でGoogle Play Billing強制が禁止 |
| **EU** | 代替決済またはIAPのどちらか一方のみ選択可 | 代替決済の選択肢あり |

外部決済を採用した場合のデメリット：
- AppleやGoogleへの手数料が引き続き発生する場合がある（米国でも12〜27%程度）
...
`CustomerInfo` の以下フィールドで状態を判断できます：

| フィールド | 意味 |
|---|---|
| `entitlements.active` | アクセス権あり（Grace Period中も`active`のまま） |
| `subscriptions[x].billingIssueDetectedAt` | 課金失敗を検知した日時（nilでなければ問題あり） |
| `subscriptions[x].gracePeriodExpiresDate` | Grace Period終了日時 |
...
- RevenueCatを使う場合、`CustomerInfo` の `entitlements.active` と `subscriptions` を参照すれば自動的に最新プランを反映可能
...
- RevenueCatを使う場合: 自動で `entitlements.active` が更新されるため、アプリ側は状態を参照するだけでよい
...
| 判断基準 | `in_app_purchase` | RevenueCat（`purchases_flutter`） |
|---|---|---|
| 外部サービス依存を最小化したい | ✅ 向いている | ❌ SaaS依存あり |
| 開発リソースが少ない・迅速に実装したい | △ 実装量多め | ✅ 向いている |
| iOS/Android共通のサブスク状態管理が必要 | △ 自前実装が必要 | ✅ 統一APIで管理可能 |
| ダッシュボード・売上分析が欲しい | ❌ 別途実装が必要 | ✅ 標準搭載 |
| バックエンドサーバーを持てない | △ 検証省略のリスクあり | ✅ RevenueCatが代替 |
| MTR $2,500以下の小規模運用 | — | ✅ 無料枠で利用可能 |
| コスト最優先（大規模運用） | ✅ ランニングコスト0 | △ 1%課金が負担になる場合あり |

### 14-2. バックエンド有無の判断基準

| 要件 | バックエンド必要？ | RevenueCatで代替可能？ |
|---|---|---|
| レシート検証（不正防止） | ✅ 必要 | ✅ 代替可能 |
| サブスク状態の永続化 | ✅ 必要 | ✅ 代替可能 |
| 複数デバイス間の権利同期 | ✅ 必要 | ✅ 代替可能 |
| Webhook受信（ストアサーバー通知） | ✅ 必要 | ✅ RevenueCatが中継 |
| 独自の権利ロジック（チーム共有など） | ✅ 必要 | △ 一部のみ |
| 決済データの独自分析・CRM連携 | ✅ 必要 | △ Webhook転送で部分対応 |
...
| 本番ビルド要件 | iOS 26 SDK（Xcode 26）必須（2026/4/28〜） | targetSdk 35以上（2025/8/31〜） |
...
| 🔴 必須 | レシート検証（サーバーまたはRevenueCat） | 不正利用防止 |
| 🔴 必須 | Grace Period有効化（特にiOS） | フリートライアル後の課金失敗で即停止を防ぐ |
| 🔴 必須 | サブスク状態のリアルタイム反映 | Webhook or RevenueCat連携 |
...
| 🟢 任意 | 売上ダッシュボード（RevenueCat等） | ビジネス指標の可視化 |

`billingIssueDetectedAt` が `null` でない場合に「支払い情報を更新してください」バナーを出す実装が最もシンプルです。
