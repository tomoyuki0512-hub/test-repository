...
> [ケーブル・コードと接続部材ガイド](optical-fiber-cable-types.md) を参照。
...
```
① 回線の用途を決める            （幹線/メトロ/アクセス/宅内/DC内）
        ↓
② 光ファイバ品種を選ぶ          §3（G.652/654/657・曲げ半径・MFD）
        ↓
③ 光ケーブルを選ぶ              §4（屋外/屋内/ドロップ/テープスロット/超多心）
        ↓
④ コネクタ/コードを選ぶ          §5（SC/LC/MPO・現場組立 e-SC/Quick）
        ↓
⑤ 接続方法・工具を選ぶ          §6（融着接続機・カッタ）
        ↓
⑥ 収容部材を選ぶ                §7（成端箱/接続箱/クロージャ）
        ↓
⑦ 高密度・DC配線は             §8（MPOカセット/超多心ソリューション）
```
...
```
許容曲げが緩い  ──────────────▶  許容曲げが厳しい（狭所）
 PureBand(標準) → PureAccess-PB(R15) → -A2(R7.5) → -R5(R5)
```
