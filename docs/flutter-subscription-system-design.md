...
【Webhook受信】
  iOS:     POST /webhook/apple         ← App Store Server Notifications（JWS）
  Android: POST /webhook/google/pubsub ← Google Cloud Pub/Sub（Push型）
```
...
App Store Server Notifications V2を受信する。
...
Google Cloud Pub/SubのPush配信を受信する。
...
| ログ出力 | purchaseToken / transactionIdをアプリログに出力しない |
